...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="urn:hl7-org:v3" version="1.0">
    ...
    <xsl:template match="/" mode="message">
        <xsl:call-template name="UnfilledPrescriptionProfileRequest">
            ...
            <xsl:with-param name="status-1" select="$status-1"/>
            <xsl:with-param name="status-2" select="$status-2"/>
            <xsl:with-param name="status-3" select="$status-3"/>
