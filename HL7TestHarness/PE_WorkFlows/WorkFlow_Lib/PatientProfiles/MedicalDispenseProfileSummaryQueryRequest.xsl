...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="urn:hl7-org:v3" version="1.0">
    ...
    <xsl:template match="/" mode="message">
        <xsl:call-template name="MedicalDispenseProfileSummaryRequest">
            ...
            <xsl:with-param name="mostRecentDispenses" select="$mostRecentDispenses"/>
            <xsl:with-param name="mostRecentDrug" select="$mostRecentDrug"/>
            <xsl:with-param name="issueFilterCode" select="$issueFilterCode"/>
