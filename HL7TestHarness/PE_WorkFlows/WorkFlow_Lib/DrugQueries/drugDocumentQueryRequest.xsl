...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/" mode="message">
        ...
        <xsl:call-template name="drugDocumentQueryRequest">
            ...
            <xsl:with-param name="symptom-code" select="$symptom-code"/>
            <xsl:with-param name="symptom-codeSystem" select="$symptom-codeSystem"/>
            <xsl:with-param name="diagnosis-code" select="$diagnosis-code"/>
            <xsl:with-param name="diagnosis-codeSystem" select="$diagnosis-codeSystem"/>
            <xsl:with-param name="document-type" select="$document-type"/>
            <xsl:with-param name="document-typeSystem" select="$document-typeSystem"/>
            <xsl:with-param name="document-id" select="$document-id"/>
            <xsl:with-param name="document-OID" select="$document-OID"/>
