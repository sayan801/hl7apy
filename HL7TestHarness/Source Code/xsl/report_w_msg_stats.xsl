...
<!--
 Module Header
  ...
  $Header:   L:/pvcsrep65/HL7TestHarness/archives/Source Code/xsl/report_w_msg_stats.xsl-arc   1.0   15 Feb 2007 14:54:24   mwicks  $
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:hl7="urn:hl7-org:v3">
    ...
    <xsl:template match="workflows">
        ...
        <xsl:apply-templates/>
        <div>
            <xsl:call-template name="message_Stats"/>
        </div>
        <div>
            <xsl:call-template name="message_Stats">
                <xsl:with-param name="include-testcases">true</xsl:with-param>
            </xsl:call-template>
        </div>
    </xsl:template>
...
-->
    ...
    <xsl:template name="MessageDetail">
        ...
        <table width="96%">
            <tr>
                <td colspan="6" class="subheader1">Message: <span onmouseover="this.style.cursor='hand'" onclick="">
    ...
    <xsl:template name="message_Stats">
        <xsl:param name="include-testcases">false</xsl:param>

        <xsl:variable name="MessageList" select="document('messageData.xml')"/>
        <xsl:variable name="test-node" select="ancestor-or-self::workflows"/>
        ...
        <table width="96%">
            <tr>
                <td colspan="8" class="subheader1">
                    <span onmouseover="this.style.cursor='hand'" onclick="">
                        <xsl:text>Message Stats </xsl:text>
                    </span>
                </td>
            </tr>
            <tr>
                <td>Message Name(HL7)</td>
                <td>Action</td>
                <td>Type</td>
                <td>Description</td>
                <td>Count</td>
            ...
            <xsl:for-each select="$MessageList/messages/*">
                <xsl:call-template name="messageCount">
                    <xsl:with-param name="include-testcases" select="$include-testcases"/>
    ...
    <xsl:template name="messageCount">
        ...
        <xsl:if test="$include-testcases = 'true' and count($test-node/ancestor-or-self::workflows/descendant-or-self::messageProxy/RxMessage[local-name(hl7:*) = $hl7-name]) &gt; 0">
            <tr>
                <td/>
                <td colspan="4">
                    <table width="96%">
